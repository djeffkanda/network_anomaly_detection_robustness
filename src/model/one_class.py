...
class DeepSVDD(BaseModel):
    ...
    def __init__(self, **kwargs):
        ...
        self.rep_dim = self.in_features // 3 if self.in_features >= 32 else 16
        self.name = "DeepSVDD"
        self.net = self._build_network()

    def _build_network(self):
        hdim = self.in_features // 2 if self.in_features >= 32 else 32
        return nn.Sequential(
            nn.Linear(self.in_features, hdim),
            nn.ReLU(),
            nn.Linear(hdim, self.rep_dim)
        ).to(self.device)
